...
internal class TouchUtils(val context: Context, val config: FloatConfig) {
    ...
    fun updateFloat(
        ...
        when (event.action and MotionEvent.ACTION_MASK) {
            ...
            MotionEvent.ACTION_MOVE -> {
                ...
                var x = params?.x?.plus(dx.toInt())
                var y = params?.y?.plus(dy.toInt())
                // 检测浮窗是否到达边缘
                if (x != null) {
                    x = when {
    ...
    private fun initBoarderValue(view: View, params: LayoutParams?) {
        ...
        statusBarHeight = if (location[1] > params?.y?:0) statusBarHeight(view) else 0
    ...
    private fun sideAnim(
        ...
        val end = when (config.sidePattern) {
            ...
            SidePattern.RESULT_RIGHT -> {
                isX = true
                params?.x?.plus(rightDistance)
            }
            SidePattern.RESULT_HORIZONTAL -> {
                isX = true
                if (leftDistance < rightDistance) leftBorder else params?.x?.plus(rightDistance)
            }
        ...
        val animator = end?.let { ValueAnimator.ofInt(if (isX) params?.x?:0 else params?.y?:0, it) }
    ...
    private fun initDistanceValue(params: LayoutParams?) {
        leftDistance = params?.x?.minus(leftBorder) ?: 0
        rightDistance =if (params?.x!= null)   rightBorder - params.x else 0
        topDistance = params?.y?.minus(topBorder) ?: 0
        bottomDistance =  if (params?.y!= null) bottomBorder - params.y else 0
