...
import com.customer.component.easyfloat.anim.AnimatorManager
import com.customer.component.easyfloat.data.FloatConfig
import com.customer.component.easyfloat.enums.ShowPattern
import com.customer.component.easyfloat.interfaces.OnFloatTouchListener
import com.customer.component.easyfloat.utils.DisplayUtils
import com.customer.component.easyfloat.utils.InputMethodUtils
import com.customer.component.easyfloat.utils.LifecycleUtils
import com.customer.component.easyfloat.widget.ParentFrameLayout
...
internal class FloatingWindowHelper(val context: Context, var config: FloatConfig) {
        ...
        frameLayout?.apply {
            ...
            viewTreeObserver?.addOnGlobalLayoutListener {
                ...
                if (config.layoutChangedGravity.and(Gravity.START) == Gravity.START) {
                    ...
                    params?.x = params?.x?.minus(diffChangedSize)
                    ...
                    params?.x = params?.x?.plus(diffChangedCenter)
                }
                ...
                if (config.layoutChangedGravity.and(Gravity.TOP) == Gravity.TOP) {
                    ...
                    params?.y = params?.y?.minus(diffChangedSize)
                    ...
                    params?.y = params?.y?.plus(diffChangedCenter)
                }
    ...
    private fun setGravity(view: View?) {
        ...
        val statusBarHeight = if (location[1] > params?.y?:0) DisplayUtils.statusBarHeight(view) else 0
        val parentBottom =
        ...
        // 设置偏移量
        params?.x = params?.x?.plus(config.offsetPair.first)
        params?.y = params?.y?.plus(config.offsetPair.second)

        if (config.immersionStatusBar) {
            if (config.showPattern != ShowPattern.CURRENT_ACTIVITY) {
                params?.y = params?.y?.minus(statusBarHeight)
            }
        } else {
            if (config.showPattern == ShowPattern.CURRENT_ACTIVITY) {
                params?.y = params?.y?.plus(statusBarHeight)
            }
        }
        // 更新浮窗位置信息
    ...
    private fun enterAnim(floatingView: View) {
        ...
        enterAnimator = params?.let {
            windowManager?.let { it1 ->
                AnimatorManager(frameLayout!!, it, it1, config)
                    .enterAnim()?.apply {
    ...
    fun exitAnim() {
        ...
        val animator: Animator? =
            params?.let { windowManager?.let { it1 ->
                AnimatorManager(frameLayout!!, it,
                    it1, config).exitAnim()
            } }
    ...
    fun updateFloat(x: Int, y: Int) {
        frameLayout?.let {
            if (x == -1 && y == -1) {
                // 未指定具体坐标，执行吸附动画
                it.postDelayed({ params?.let { it1 ->
                    windowManager?.let { it2 ->
                        touchUtils.updateFloat(it,
                            it1, it2
                        )
                    }
                } }, 200)
