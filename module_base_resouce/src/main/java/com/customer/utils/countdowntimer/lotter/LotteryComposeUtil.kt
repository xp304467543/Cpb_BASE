    ...
    fun lotterySumAndType(result: List<String>): List<String> {
        ...
        repeat(3) {
            ...
            if (arr[0] == arr[1] && arr[1] == arr[2]) {
                str.add("豹子")     //豹子
            } else if ((arr[0] + 1) == arr[1] && (arr[1] + 1) == arr[2] || (arr[0] == 0 && arr[1] ==1 && arr[2] == 9)) {
                str.add("顺子")   //顺子
            } else if (arr[0] == arr[1] || arr[1] == arr[2]) {
                str.add("对子")   //对子
            } else if ((arr[0] + 1) == arr[1] || (arr[1] + 1) == arr[2] || (arr.contains(0) && arr.contains(9))) {
                str.add("半顺")   //半顺
            } else str.add("杂六")    //杂六
        }
        return str
                ...
                LotteryConstant.TYPE_20, "前三形态", "中三形态", "后三形态" -> {
                    if (dataFrom != null) {
                        for ((index, it) in data.withIndex()) {
                            ...
                            map["date"] = it.issue
                            map["num0"] = if (options1 ==0) it.open_code?.split(",")?.get(0) else if (options1 ==1) it.open_code?.split(",")?.get(1) else it.open_code?.split(",")?.get(2)
                            map["num1"] = if (options1 ==0) it.open_code?.split(",")?.get(1) else if (options1 ==1) it.open_code?.split(",")?.get(2) else it.open_code?.split(",")?.get(3)
                            map["num2"] = if (options1 ==0) it.open_code?.split(",")?.get(2) else if (options1 ==1) it.open_code?.split(",")?.get(3) else it.open_code?.split(",")?.get(4)
