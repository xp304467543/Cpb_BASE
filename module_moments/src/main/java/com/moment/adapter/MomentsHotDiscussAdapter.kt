 ...
 *
 */
class MomentsHotDiscussAdapter(val mContext: Context) :
    XRecyclerAdapter<NineGridInfo, MomentsHotDiscussAdapter.NineGridHolder>() {
    ...
    override fun bindData(holder: NineGridHolder, position: Int, data: NineGridInfo?) {
        if (data != null) {
            holder.bind(data)
        }
    }
    ...
    inner class NineGridHolder : RecyclerView.ViewHolder {
        ...
        private var imgManage: RadiusImageView? = null
        ...
        fun bind(gridInfo: NineGridInfo) {
            if (gridInfo.getIsPromote() == "1") {
                ViewUtils.setGone(linDianZan)
                ViewUtils.setGone(linReply)
                ViewUtils.setGone(mNglContent)
                ViewUtils.setVisible(imgManage)
                if (!gridInfo.getImgUrlList().isNullOrEmpty()) imgManage?.let {
                    GlideUtil.loadImage(
                        gridInfo.getImgUrlList()?.get(0)?.url,
                        it
                    )
                }

            } else {
                ViewUtils.setGone(imgManage)
                ViewUtils.setVisible(mNglContent)
            ...
            linDianZan?.setOnClickListener {
                ...
                MomentsApi.getQuizArticleLikeResult(gridInfo.getId().toString()) {
                    onSuccess {
                        gridInfo.setIsLike("1")
            ...
            imgMomentsDiscussHolderPhoto?.setOnClickListener {
                if (!FastClickUtil.isFastClick()) {
                    Router.withApi(ApiRouter::class.java)
                        .toUserPage(gridInfo.getUserId().toString())
                }
            }
        }
    }
