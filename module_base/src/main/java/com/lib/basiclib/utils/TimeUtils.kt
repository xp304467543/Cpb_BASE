...
object TimeUtils {
    ...
    private val formatYearMonthDay2 = SimpleDateFormat("yyyy-MM-dd HH:mm" , Locale.getDefault())
    ...
    private val formatYearMonthDayHourMinute = SimpleDateFormat("MM/dd HH:mm", Locale.getDefault())
    ...
    /**
     * 年月日格式的时间转换成毫秒值，格式：2018-12-12  12:12
     */
    fun getMillisecondByDate2(date: String): Long {
        return formatYearMonthDay2.parse(date).time
    }
    ...
    @SuppressLint("SimpleDateFormat")
    fun date2TimeStampOnlyTime(date: String?): String? {
        try {
            val sdf = SimpleDateFormat("yyyy-MM-dd HH:mm")
    ...
    @SuppressLint("SimpleDateFormat")
    fun date2TimeStampOnlyYear(date: String?): String? {
        try {
            val sdf = SimpleDateFormat("yyyy-MM-dd HH:mm")
     ...
     * long 类型转换成日期
     */
    fun longToDateStringYMDHM(long: Long): String? {
        val date = Date(long * 1000)
    ...
    */
    ...
    private val format5 = SimpleDateFormat("mm:ss", Locale.getDefault())

    fun longToDateStringTimeMS(long: Long): String? {
        val date = Date(long )
    ...
    fun setTime(millisUntilFinished: Long) :String?{
        ...
        val second: Long =
           ...
           return when {
                day > 0 ->  dataLong(day) + "天" + dataLong(hour) + ":" + dataLong(minute) + ":" + dataLong(second)
                hour > 0 ->  dataLong(hour) + ":" + dataLong(minute) + ":" + dataLong(second)
                else ->  dataLong(minute) + ":" + dataLong(second)
            }
    }

    @SuppressLint("SetTextI18n")
    fun setTimeRedRain(millisUntilFinished: Long) : String {
        val day: Long = millisUntilFinished / (1000 * 60 * 60 * 24)/*单位 天*/
        val hour: Long = (millisUntilFinished - day * (1000 * 60 * 60 * 24)) / (1000 * 60 * 60)/*单位 时*/
        val minute: Long =
            (millisUntilFinished - day * (1000 * 60 * 60 * 24) - hour * (1000 * 60 * 60)) / (1000 * 60)/*单位 分*/
