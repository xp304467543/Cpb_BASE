...
open class BaseNavActivity(override val layoutResID: Int = R.layout.base_nav_page) :
    ...
    final override fun initView() {
        if (!isOverride()) {
            ...
            if (isShowToolBar()) {
                ...
                if (isShowRightText())setVisible(barRightText)
                // 左侧返回按钮
    ...
    protected open fun isShowRightText():Boolean = false
    ...
    protected fun setRightText(text:String?){
        ...
        setVisible(barRightText)
    }

    protected fun getRightText():AppCompatTextView{
        return  barRightText
    }
    ...
    protected fun setPageTitleColor(title: Boolean) {
        if (title) tvTitle.setTextColor(ViewUtils.getColor(R.color.white)) else tvTitle.setTextColor(ViewUtils.getColor(R.color.blueSport))
