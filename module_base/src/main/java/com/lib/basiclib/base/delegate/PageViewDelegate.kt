...
class PageViewDelegate(private val view: View?) {
    ...
    fun setBackIcon(toolbar: Toolbar?, mainPage: Boolean, backIcon: Boolean,backColorIcon: Boolean,isClose:Boolean, backClick: () -> Unit) {
        if (backIcon && !mainPage) {
            val icon  = if(isClose) R.drawable.ic_close else { if (backColorIcon) R.mipmap.ic_arrow_back_white else R.mipmap.ic_arrow_back_black }
            toolbar?.navigationIcon = ViewUtils.getImageThemeDrawable(toolbar?.context, icon, R.attr.toolbarForeground)
