...
import com.player.ali.alivcplayerexpand.util.RegularPatternUtil;
...
public class AliyunPlayerConfigActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private void saveConfig(){
        ...
        GlobalPlayerConfig.PlayConfig.mMaxDelayTime = TextUtils.isEmpty(maxDelayTime) ? GlobalPlayerConfig.PlayConfig.DEFAULT_MAX_DELAY_TIME : Integer.parseInt(maxDelayTime);
        String probeSize = mProbeSizeEditText.getText().toString();
        if(!RegularPatternUtil.isNumber(probeSize)){
            probeSize = "-1";
        }
        GlobalPlayerConfig.PlayConfig.mMaxProbeSize = TextUtils.isEmpty(probeSize) ? 0 : Integer.parseInt(probeSize);
        String netWorkTimeOut = mNetWorkTimeOutEditText.getText().toString();
        GlobalPlayerConfig.PlayConfig.mNetworkTimeout = TextUtils.isEmpty(netWorkTimeOut) ? GlobalPlayerConfig.PlayConfig.DEFAULT_NETWORK_TIMEOUT : Integer.parseInt(netWorkTimeOut);
        String retryCount = mRetryCountEditText.getText().toString();
        GlobalPlayerConfig.PlayConfig.mNetworkRetryCount = TextUtils.isEmpty(retryCount) ? GlobalPlayerConfig.PlayConfig.DEFAULT_NETWORK_RETRY_COUNT : Integer.parseInt(retryCount);
        String maxBufferDuration = mMaxBufferPacketDurationEditText.getText().toString();
        GlobalPlayerConfig.PlayConfig.mMaxBufferDuration = TextUtils.isEmpty(maxBufferDuration) ? GlobalPlayerConfig.PlayConfig.DEFAULT_MAX_BUFFER_DURATION : Integer.parseInt(maxBufferDuration);
        String heightBufferLevel = mHightBufferLevelEditText.getText().toString();
        GlobalPlayerConfig.PlayConfig.mHighBufferDuration = TextUtils.isEmpty(heightBufferLevel) ? GlobalPlayerConfig.PlayConfig.DEFAULT_HIGH_BUFFER_DURATION : Integer.parseInt(heightBufferLevel);
        String firstStartBufferLevel = mFirstStartBufferLevelEditText.getText().toString();
        GlobalPlayerConfig.PlayConfig.mStartBufferDuration = TextUtils.isEmpty(firstStartBufferLevel) ? GlobalPlayerConfig.PlayConfig.DEFAULT_START_BUFFER_DURATION : Integer.parseInt(firstStartBufferLevel);
        ...
        GlobalPlayerConfig.PlayCacheConfig.mMaxSizeMB = TextUtils.isEmpty(maxSizeMB) ? GlobalPlayerConfig.PlayCacheConfig.DEFAULT_MAX_SIZE_MB : Integer.parseInt(maxSizeMB);
        String maxDuration = mMaxDurationEditText.getText().toString();
        GlobalPlayerConfig.PlayCacheConfig.mMaxDurationS = TextUtils.isEmpty(maxDuration) ? GlobalPlayerConfig.PlayCacheConfig.DEFAULT_MAX_DURATION_S : Integer.parseInt(maxDuration);
    }
}
